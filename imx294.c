				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		try_fmt->code = imx294_get_format_code(imx294, try_fmt->code);
		fmt->format = *try_fmt;
	} else {
		fmt->format.width        = imx294->mode->width;
        fmt->format.height       = imx294->mode->height;
...
static int imx294_start_streaming(struct imx294 *imx294)
{
    ...
    ret = cci_multi_reg_write(imx294->regmap, imx294->mode->reg_list.regs,
                  imx294->mode->reg_list.num_of_regs, NULL);
    if (ret) {
...
{
    struct imx294 *imx294 = to_imx294(sd);
    struct v4l2_mbus_framefmt *try_fmt_img = v4l2_subdev_state_get_format(fh->state, 0);
    ...
    try_crop->left = supported_modes_12bit[0].crop.left;
    try_crop->top = supported_modes_12bit[0].crop.top;
    try_crop->width = supported_modes_12bit[0].crop.width;
    try_crop->height = supported_modes_12bit[0].crop.height;
...
static int imx294_init_controls(struct imx294 *imx294)
{
   ...
   /* Read-only, updated per mode */
	...
	ret = v4l2_ctrl_new_fwnode_properties(hdl, &imx294_ctrl_ops,
					      &props);
	if (ret)
		goto error;
